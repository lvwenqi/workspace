...
(function(w){
	...
	// }(function(s){
	// 	console.log(s)
	// 	var _buildLib = '<link rel="stylesheet" href="'+s+'css/base.css'+(_debug ? '?v=' + (+new Date()) : '')+'">';
	...
	var _buildEditorModule = function(_toolbars, _uid){
		...
		_html += '		</ul>\
					...
					<div class="Eleditor-textEditor style="display:block;">\
	...
	w[_namespace] = function(){
		...
		var _args = arguments[0];
			...
			_historys = [],
			_placeHolder = _args.placeHolder || '<p class="Eleditor-placeholder">点击 “插文字” 开始编译文章</p>',
			_uploadRole = null;
		...
		_showEditorControllerLayer($('p.Eleditor-placeholder'))
		/*call*/
        _args.mounted();
